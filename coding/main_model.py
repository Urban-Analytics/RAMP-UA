...
def main(parameters_file):
    ...
    if not use_cache or cache.is_empty():
        print(f'You will need to run the main_initalisation modulw because {"caching is disabled" if not use_cache else "the cache is empty"}')
    ...
    if lockdown_file != "":
        ...
        time_activity_multiplier: pd.DataFrame = \
            PopulationInitialisation.read_time_activity_multiplier(path + lockdown_file)
...
def run_opencl_model(individuals_df,
                     ...
                     calibration_params,
                     disease_params):
    snapshot_cache_filepath = () # project_dir + "microsim/opencl/snapshots/cache.npz"
