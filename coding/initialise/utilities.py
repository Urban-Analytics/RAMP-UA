...
def check_durations_sum_to_1(individuals, activities):
    epsilon = 0.01
    total_dura = [0.0] * len(individuals)  # Add up all the different activity durations
    for activity in activities:
        total_dura = total_dura + individuals.loc[:, f"{activity}{ColumnNames.ACTIVITY_DURATION}"]
    if not (total_dura.apply(lambda x: round(x, 2)) == 1.0).all():
        print("Some activity durations don't sum to 1", flush=True)
        print(total_dura[total_dura != 1.0], flush=True)
